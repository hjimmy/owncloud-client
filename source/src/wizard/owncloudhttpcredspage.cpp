...
//added by jian.hou:set Default Username and Password
void OwncloudHttpCredsPage::setDefaultUsernamePwd()
{
#ifdef Q_OS_LINUX 
   QFile f("/home/.Autologin.txt");
#else
   #ifdef Q_OS_WIN32
      QFile f("C:\\Autologin.txt");
   #else
      QFile f;
   #endif
#endif
   if(f.open(QIODevice::ReadOnly | QIODevice::Text)){
        ...
        username = lineStr.section(';', 0,0);
        username = username.section(":",1,1);
        password = lineStr.section(';', 1,1);
        password = password.section(":",1,1);
...
{
    //added by jian.hou Don't clear username and password
    _ui.leUsername->clear();
    _ui.lePassword->clear();
}
...
bool OwncloudHttpCredsPage::isComplete() const
{
    if (_ui.leUsername->text().isEmpty() || _ui.lePassword->text().isEmpty()){
        return false;
    }
